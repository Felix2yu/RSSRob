...
| 键 | 默认值 | 说明 |
|-----|---------|-------------|
| `output_dir` | `./var/feeds` | 生成的 `<name>.xml` 文件写入并对外提供的目录。 |
| `state_db` | `./var/rssrob.db` | 存放条目历史 / 去重状态的 SQLite 文件。 |
...
| `type` | 否 | `html`（默认）、`rss`、`wechat` 或 `twitter`，决定数据源处理方式。 |
...
├── docs/                     # 设计文档与计划
└── var/                      # 运行时状态（已 gitignore）—— 首次运行时自动创建
    ├── feeds/                # 生成的 <name>.xml，由 HTTP 服务器对外提供
    ├── rssrob.db             # SQLite 条目历史 + 去重状态
    ├── subscribers.json      # 各订阅源的邮件订阅者
    ├── digest_state.json     # 「已发送」条目 id（增量摘要）
    ├── wechat_credential.json # 公众号会话 cookie + token（个人密钥）
    └── twitter_credential.json # X 会话 cookie（auth_token + ct0；个人密钥）
```
...
- `/wechat/login` —— 登录你的公众号后台，搜索并添加公众号订阅源。
- `/twitter/login` —— 粘贴你的 x.com cookie，把 X 账号添加为订阅源。
- `/backup` —— **备份 / 恢复**：把配置与 `var/` 下的全部运行时状态（SQLite 历史、生成的订阅源、订阅者、digest 状态、公众号凭据）打包下载为一个 `.zip`，可在另一台实例或重装后恢复。该 zip 含密钥，请妥善保管；恢复后请重启 `rssrob serve`。

---

## 微信订阅号（公众号）

公众号没有公开 RSS，也封锁了对文章列表的抓取。RSSRob 改为通过 **你自己注册的公众号
后台**（mp.weixin.qq.com）来获取：其图文编辑接口可以按名称搜索任意公众号，并列出该号
已发布的文章。登录一次、按名称搜索并保存，RSSRob 之后便像普通订阅源一样按计划轮询。

> **需要你自己的公众号。** 你用 *自己* 注册的公众号登录 mp.weixin.qq.com；RSSRob 只读、
> 绝不发布。微信对这些接口有频率限制，请礼貌轮询。平台调用都集中在
> `rssrob/wechat.py`（`MpPlatformTransport`）。
>
> （最初评估过微信读书，但其接口根本不暴露公众号，因此改用公众号平台。）

### 0. 注册一个公众号（如果还没有）

这个账号只是解锁“文章搜索接口”的钥匙——你完全不需要用它发布任何内容。

1. 打开 <https://mp.weixin.qq.com/> → **立即注册**。
2. 选择 **订阅号**（免费）。主体可选 **个人**，只需中国身份证 + 银行卡/人脸核验。
3. 完成邮箱与身份验证后登录。

### 1. 登录 mp.weixin.qq.com

RSSRob 从浏览器正常登录后获取会话 **cookie + token**：

1. 打开 <https://mp.weixin.qq.com/> 登录你自己的公众号。（`rssrob wechat-login` 不带参数
   时、以及 `/wechat/login` 页面，都会渲染一个指向该地址的二维码。）
2. 登录后从地址栏复制 **token**（`…&token=123456789`，只要那串数字）。
3. 复制 **cookie**——即完整的 `Cookie:` 请求头：DevTools(F12) → **Network** → 点任意一个
   发往 `mp.weixin.qq.com` 的请求 → **Headers** → **Request Headers** → 复制整段
   `Cookie` 值。（不要用 `document.cookie`——关键登录 cookie 是 `HttpOnly`，那里看不到。）
4. 保存。最简单的是交互式粘贴（不必在命令行里给超长 cookie 加引号）：

   ```bash
   rssrob wechat-login        # 依次提示你粘贴 token、cookie
   ```

   也可用参数传入，或用网页表单：

   ```bash
   rssrob wechat-login --token 123456789 --cookie "<cookie 字符串>"
   ```

   ……或把两者粘贴到 Web 应用的 **/wechat/login** 页面。

凭据保存在 `var/wechat_credential.json`（已被 gitignore——属于个人密钥）。会话过一段时间会
失效，届时会清晰提示重新登录；重新粘贴即可刷新。
...
```bash
rssrob wechat-search "某某公众号"               # 列出匹配项及其 id
rssrob wechat-search "某某公众号" --save my-oa  # 选择其一；写入 configs/my-oa.yaml
```

这会写入一个 `type: wechat` 的订阅源配置：

```yaml
name: my-oa
type: wechat
account_id: "MzAx...=="     # 公众号的 fakeid（由搜索填入）
account_name: "某某公众号"   # RSS <title> 的默认值
interval: 7200              # 请礼貌轮询，不要过于频繁
max_items: 50
```

每条目为文章的 **标题 + 发布时间 + 摘要**，正文链接指向 `mp.weixin.qq.com`。Web 应用的
`/wechat/login` 页面也内置了搜索框，可直接帮你保存订阅源。
...
|----|------|------|
| `type` | 是 | 必须为 `wechat`。 |
| `account_id` | 是 | 公众号的 `fakeid`；由 `wechat-search` 写入。 |
| `account_name` | 否 | 人类可读名称；作为 RSS `<title>` 默认值。 |
| `interval` / `max_items` | 否 | 与其他订阅源相同。建议 `interval` 较长（≥ 7200 秒）以避免被 mp.weixin.qq.com 限流。 |

`wechat` 不使用 `url`、`item`、`fields`。所有公众号订阅源共用一个平台客户端，因此请求
会被串行化并保持间隔；遇到限流会自动退避，会话过期时会清晰地提示 “run `rssrob
wechat-login`” 并继续提供上一次成功生成的订阅源。

---

## Twitter / X 订阅源

X（Twitter）没有免费的公开 RSS，账号也藏在 JS 应用和登录墙之后。RSSRob 改为通过
**你自己已登录的 X 会话**来读取：它调用 x.com 网页端使用的同一套内部 GraphQL 接口，
携带你的会话 cookie —— 无需付费 API，也无需无头浏览器。

> **使用你自己的 X 会话。** 你从已登录的 x.com 粘贴 cookie；RSSRob 只读、绝不发帖。
> 这在技术上违反 X 的服务条款，且会话 cookie 会过期（重新粘贴即可），请按个人用途使用。
> HTTP 层隔离在 `rssrob/twitter.py`（`GraphQlTransport`）。X 还会定期轮换 GraphQL
> query-id；若请求开始报 “query-id rotated” 错误，请更新 `GraphQlTransport.QUERY_IDS`。

### 1. 获取 X cookie

登录 <https://x.com/>，然后复制你的 **`Cookie:` 请求头**：

DevTools（F12）→ **Network** → 点击任意 `x.com` 请求 → **Headers** →
**Request Headers** → 复制整段 `Cookie` 值。（别用 `document.cookie`——关键的
`auth_token` 是 `HttpOnly`。）RSSRob 会从中提取它需要的两个 cookie（`auth_token` 和 `ct0`）。

```bash
rssrob twitter-login                                  # 提示你粘贴 cookie
rssrob twitter-login --cookie "<cookie>" --proxy 7890 # 非交互；可选代理
```

……或粘贴到 Web 应用的 **/twitter/login** 页面。凭据保存在
`var/twitter_credential.json`（已被 gitignore——属于个人密钥）。

### 2. 把账号添加为订阅源

```bash
rssrob twitter-add elonmusk                  # 显示解析到的账号
rssrob twitter-add elonmusk --save elon      # 保存；写入 configs/elon.yaml
```

它写入一个 `type: twitter` 订阅源配置：

```yaml
name: elon
type: twitter
username: elonmusk
account_id: "44196397"     # 解析得到的 rest_id；省去每次抓取的查询
account_name: "Elon Musk"  # RSS <title> 的默认值
```

`/twitter/login` 页面也内置了“添加账号”表单。
...
|----|------|------|
| `type` | 是 | 必须为 `twitter`。 |
| `username` | 是 | `@handle`（不含 `@`）。 |
| `account_id` | 否 | 数字 `rest_id`；由 `twitter-add` 写入以省去 handle 查询。 |
| `interval` / `max_items` | 否 | 与其他订阅源相同。 |

`twitter` 不使用 `url`、`item`、`fields`。所有 twitter 订阅源共用一个客户端；代理是
全局的（设置 `RSSROB_PROXY`，或 `twitter-login --proxy`）。会话过期时会提示
“run `rssrob twitter-login`” 并继续提供上一次成功生成的订阅源。

> **代理说明：** 每订阅源的 `proxy:` 适用于 `html`/`rss`（现在 `rssrob serve` 路径
> 也会生效，不再只是 Web 预览）。Twitter 使用上面的全局代理。
